...
ALTER TABLE IF EXISTS case_entity ADD CONSTRAINT UK_p16mrav9n0plvp35asv819sg7 UNIQUE (uuid);

-- Add the unique constraint on uuid column in party_entity table
ALTER TABLE IF EXISTS party_entity ADD CONSTRAINT UK_sv72tcko7vo9jj47g0kyouqxa UNIQUE (uuid);

-- Add the foreign key constraint from party_entity to case_entity
ALTER TABLE IF EXISTS party_entity ADD CONSTRAINT FKjfwisfraa28bwwfxgflp8905u FOREIGN KEY (case_entity_id) REFERENCES case_entity;

-- Add the foreign key constraint from party_entity_addresses to party_entity
ALTER TABLE IF EXISTS party_entity_addresses ADD CONSTRAINT FK3e5ut2ua6nrlfs18vdxeye4s4 FOREIGN KEY (party_entity_id) REFERENCES party_entity;
